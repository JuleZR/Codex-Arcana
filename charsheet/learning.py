...
from charsheet.lesson_rules import (
    LessonRequirementContext, lesson_queryset, missing_requirement_labels,
)
...
def _process_locked_learning_submission(character: Character, post_data) -> tuple[str, str]:
    ...
    active_clerical_school_ids = {
        ...
        and is_clerical_school(school_defs[str(school_id)])
    }
    if len(active_clerical_school_ids) > 1:
        return "error", (
            "Es kann nur eine klerikale Schule gelernt werden. "
            "Das gilt auch für Kultist, Druide und Schamane."
        )
    ...
    positive_clerical_school_ids = {
        int(school_id)
        for school_id, add in school_plan.items()
        if int(add) > 0 and is_divine_entity_school(school_defs[school_id])
    }
        ...
        with transaction.atomic():
            ...
            if magic_spell_selection:
                ...
                for spell_id in magic_spell_selection:
                    ...
                    selected_slots_by_source[source_key] = selected_slots_by_source.get(source_key, 0) + int(
                        row.get("slot_cost", 1) or 1
                    )
                    source_limits[source_key] = (
                        str(row.get("slot_source_name") or "Zauberquelle"),
                        int(row.get("slot_source_remaining", 0) or 0),
                    )
                for source_key, selected_count in selected_slots_by_source.items():
                    source_name, remaining_count = source_limits[source_key]
                    if selected_count > remaining_count:
                        raise LearningSubmissionError(
                            f"{source_name}: Nicht genug verfuegbare gebundene Zauber-Slots."
                        )
            ...
            lesson_defs.update({
                lesson.pk: lesson
                for lesson in lesson_queryset().filter(pk__in=addition_ids)
            })
